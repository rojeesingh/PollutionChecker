...
import { baseServerUrl1 } from '../constants';
@Injectable({
  providedIn: 'root',
})
export class WishlistService {
  ...
  addCityToWishList(requestData: CityRequestData): Observable<CityRequestData> {
    const url = baseServerUrl1 + '/wishlist/add';
    const observable: Observable<CityRequestData> =
      this.client.post<CityRequestData>(url, requestData);
    return observable;
  }

  getWishList(id: any): Observable<CityRequestData[]> {
    const url =
      baseServerUrl1 + '/wishlist/byid/' + this.authService.getUsername();
    const observable: Observable<CityRequestData[]> =
      this.client.get<CityRequestData[]>(url);
    return observable;
  }

  deleteFromWishList(requestData: RequestDeleteData): any {
    const url = baseServerUrl1 + '/wishlist/delete';
    const headers = {
      body: requestData,
    };
    const observable: Observable<RequestDeleteData> =
      this.client.delete<RequestDeleteData>(url, headers);
    return observable;
  }

  fetchCityFromExternalServer(
    cityName: string,
    stateName: string,
    countryName: string
  ): Observable<any> {
